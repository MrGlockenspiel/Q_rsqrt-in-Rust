...
fn main() {
    ...
    // Take input and convert it to an f32 in one beautiful command
    let input_as_float = io::stdin()
        .lock()
        .lines()
        .next()
        .unwrap()
    ...
    println!("\n{}", q_rsqrt_result);
    println!(
        "The fast inverse square root took {:?}",
        q_rsqrt_end.duration_since(q_rsqrt_start)
    );

    // Print results of the regular way
    println!("\n{}", standard_rsqrt_result);
    println!(
        "The standard 1/sqrt(x) took {:?}",
        standard_rsqrt_end.duration_since(standard_rsqrt_start)
    );
}
